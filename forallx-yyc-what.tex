...
%O assunto da lógica é a avaliação de argumentos.
...
O mesmo argumento também poderia ter sido apresentado com a conclusão no meio:
	...
	\end{quote}
O importante aqui é percebermos que um texto qualquer será um argumento sempre que ele for a expressão de um raciocínio, onde chega-se a uma conclusão a partir de uma ou mais premissas. 
...
Chamamos de \define{invalid} qualquer argumento em que, como este, a conclusão não se segue das premissas.
...
Chamamos uma situação como esta, que prova a invalidade de um argumento, de \define{counterexample} do argumento.
...
%	\factoidbox{
%		Uma sentença $A$ é \xdefine{consequencia} das sentenças $B_1$, \dots, $B_n$ se e somente se não houver nenhuma situação em que $B_1$, \dots, $B_n$ sejam todas verdadeiras e $A$ não seja. (Também dizemos que $A$ \xdefine{se segue de} $B_1$, \dots, $B_n$ ou que $B_1$, \dots, $B_n$ \xdefine{sustentam}~$A$.)
%	}
	\factoidbox{
		Uma sentença $A$ é \define{consequence} das sentenças $B_1$, \dots, $B_n$ se e somente se não há situação em que $B_1$, \dots, $B_n$ sejam todas verdadeiras e $A$ seja falsa.
   }
...
Mas essa ``definição'' ainda está incompleta. Ela não nos diz o que é uma ``situação'' ou o que significa ser ``verdadeiro em uma situação''.
...
Como já apresentamos uma primeira aproximação de uma definição de consequência, vamos utilizá-la para registrar as definições de argumento válido e inválido:
	...
	\factoidbox{
		Um argumento é \define{valid} se e somente se a conclusão é uma consequência das premissas.
	}

	\factoidbox{
		Um argumento é \define{invalid} se e somente se ele não é válido, ou seja, se ele possui um contraexemplo.
	}

Uma das tarefas dos lógicos é tornar esta noção de ``situação'' mais precisa e investigar em que medida diferentes modos de tornar mais precisa a noção de ``situação'' afetam quais argumentos serão classificados como válidos e quais não serão. 
...
Podemos chamar de \define{nomologicamente valido}\label{nomoval} um argumento para o qual não há contraexemplos que não violem as leis da natureza.
E podemos chamar de \define{conceitualmente valido} um argumento para o qual não há contraexemplos que não violem as conexões de nossos conceitos.
...
Além disso, este primeiro argumento não é formalmente válido. \emph{Sua} forma é:
\begin{earg}
	\item[] $A$ é um $X$ ou um $Y$.
	\item[] $A$ não é um $Y$.
...
Diremos que um argumento válido com todas as suas premissas verdadeiras é um argumento \definepl{soundness}.
